...
// =============================================================================
// SISTEMAS AVANZADOS DE SEGURIDAD Y ANALYTICS
// =============================================================================
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://spainrp-oficial.onrender.com"],
...
const globalRateLimit = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    console.log(`[RATE_LIMIT] Blocked IP ${req.ip} - Too many requests`);
    res.status(429).json({
      error: 'Demasiadas solicitudes desde esta IP, intenta de nuevo más tarde.',
      retryAfter: '15 minutos'
    });
...
const speedLimiter = slowDown({
  ...
  onLimitReached: (req, res, options) => {
    console.log(`[SLOW_DOWN] IP ${req.ip} reached slow down limit`);
  }
});

// Aplicar rate limiting y slow down
...
// =============================================================================
// NUEVAS RUTAS DE API PARA SISTEMAS AVANZADOS
// =============================================================================
...
app.post('/api/analytics/search', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error registrando búsqueda' });
  }
});
...
app.get('/api/security/stats', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error obteniendo estadísticas de seguridad' });
  }
});
...
app.post('/api/backup/create', async (req, res) => {
  try {
    ...
    if (type === 'full') {
      ...
      result = await backup.createDatabaseBackup();
    }
...
app.post('/api/backup/restore', async (req, res) => {
  try {
    ...
    if (!backupPath) {
      return res.status(400).json({ error: 'Ruta de backup requerida' });
    }
...
app.get('/api/health/detailed', (req, res) => {
  try {
    const metrics = health.getDetailedMetrics();
    res.json(metrics);
  } catch (error) {
    console.error('[HEALTH] Error getting detailed metrics:', error);
    res.status(500).json({ error: 'Error obteniendo métricas detalladas' });
  }
});

app.get('/api/health/alerts', (req, res) => {
  try {
    const alerts = health.getAlerts();
    res.json(alerts);
  } catch (error) {
    console.error('[HEALTH] Error getting alerts:', error);
    res.status(500).json({ error: 'Error obteniendo alertas' });
  }
});

app.post('/api/health/alerts/clear', (req, res) => {
  try {
    health.clearAlerts();
    res.json({ success: true, message: 'Alertas limpiadas' });
  } catch (error) {
